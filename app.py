...
SUPPORTED_LANGUAGES = {
    ...
    "EL": "Greek", "EN-GB": "English (British)", "EN-US": "English (American)",
    ...
    "PT-PT": "Portuguese (all Portuguese varieties mixed)", "RO": "Romanian",
...
def translate_pdf(source_path, output_path, target_lang):
    ...
    """
    try:
        # Используем функцию из библиотеки DeepL для перевода всего документа
        deepl_client.translate_document_from_filepath(
            source_path,
            output_path,
            target_lang=target_lang,
            # preserve_formatting=True is the default behavior
        )
        print(f"Перевод завершён. Сохранён файл: {output_path}")
    except deepl.DocumentTranslationException as error:
        # Если ошибка после загрузки, можно получить handle для поддержки
        print(f"DocumentTranslationException: {error}")
        print(f" ID: {error.document_handle.id}, Key: {error.document_handle.key}")
        raise error
    except deepl.DeepLException as error:
        # Общие ошибки DeepL API
